...
void green_keeper_periodic()
{
  ...
  safeToGoForwards = col_count[3] < tresholdColorCount;
  VERBOSE_PRINT("cnt1= %d   cnt2 = %d   cnt3 = %d cnt4 = %d  cnt=5%d,   cnt6=%d,    cnt7%d \n", col_count[0], col_count[1], col_count[2], col_count[3], col_count[4], col_count[5], col_count[6]);
  ...
  if(safeToGoForwards){
      ...
      uint16_t max = findMax(col_count, sizeof(array)/sizeof(array[0])
...
* 
*/
int32_t findSafeRelativeHeading(uint32_t *array, uint8_t size){
	uint16_t max = findMax(array, size);
	uint8_t index = findIndex(array, size, max);
	int32_t relHead;
	if(index==0){relHead = -52;}
	if(index==1){relHead = -37;}
	if(index==2){relHead = -22;}
	if(index==3){relHead = 0;}
	if(index==4){relHead = 22;}
	return relHead;


	
/*
*   Find maximum in colorcount array
*/
uint32_t findMax(uint32_t *array, uint8_t size){
...
/*
*Find index of value array
*/
uint8_t findIndex(uint32_t *array, uint8_t size, uint32_t value){
	uint8_t index = 3;
	for(uint8_t i=0; i<size; i++){
		if(array[i] == value){
			index = i;

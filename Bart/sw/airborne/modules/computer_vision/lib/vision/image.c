...
else if (x == (uint32_t)(2*(output->w)/8)){
	col[1] = cnt - col[0];
	}
else if (x == (uint32_t)(3*(output->w)/8)){
	col[2] = cnt - col[1] - col[0];
	}
else if (x == (uint32_t)(5*(output->w)/8)){
	col[3] = cnt - col[0] - col[1] - col[2];
	}
else if (x == (uint32_t)(6*(output->w)/8)){
	col[4] = cnt - col[0] - col[1] - col[2] - col[3];
	}
else if (x == (uint32_t)(7*(output->w)/8)){
	col[5] = cnt - col[0] - col[1] - col[2] - col[3] - col[4];
	}
//removed last if statement, didn work. Instead it will now fill col3 once the entire filter has ran.
else if (x == ((output->w)) -1 ){
	col[6] = cnt - col[0] - col[1] - col[2] - col[3] - col[4] - col[5];
	}
  }
